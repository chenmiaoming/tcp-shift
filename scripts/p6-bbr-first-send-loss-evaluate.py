...
if len(sys.argv) != 3:
    raise SystemExit(
        "usage: p6-bbr-first-send-loss-evaluate.py "
...
if ts_loss < 1 or ts_loss > expected:
    raise SystemExit(
        f"tcp-shift recovery episode count outside bounded range: "
        f"loss={ts_loss} expected_drops={expected}"
    )
if ts_timeout != 0:
    raise SystemExit(f"tcp-shift fell back to RTO: {ts_timeout}")
...
print(
    "p6_bbr_first_send_loss_comparison=ok "
...
print("tcp_shift_first_send_loss_summary=" + ts_text)
print("linux_bbr_first_send_loss_summary=" + linux_text)
