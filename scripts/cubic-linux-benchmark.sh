...
SEED=${TCP_SHIFT_CUBIC_BENCH_SEED:-424242}
...
command -v tc >/dev/null 2>&1 || {
    echo "tc is required for CUBIC benchmark" >&2
    exit 1
}
command -v ip >/dev/null 2>&1 || {
    echo "ip is required for CUBIC benchmark" >&2
    exit 1
}
command -v python3 >/dev/null 2>&1 || {
    echo "python3 is required for CUBIC benchmark" >&2
    exit 1
}
...
case "$SEED" in ''|*[!0-9]*) echo "SEED must be an integer" >&2; exit 1;; esac
[ "$RTT_MS" -gt 0 ] && [ $((RTT_MS % 2)) -eq 0 ] || {
    echo "RTT_MS must be a positive even integer so delay can be split symmetrically" >&2
    exit 1
}
[ "$RATE_MBIT" -gt 0 ] || { echo "RATE_MBIT must be positive" >&2; exit 1; }
[ "$PAYLOAD_BYTES" -gt 0 ] || { echo "PAYLOAD_BYTES must be positive" >&2; exit 1; }
case "$LOSS_PCT" in
    ''|*[!0-9.]*|*.*.*) echo "LOSS_PCT must be a nonnegative decimal percentage" >&2; exit 1;;
esac
...
add_data_netem()
{
    ...
    if [ "$LOSS_PCT" = 0 ] || [ "$LOSS_PCT" = 0.0 ]; then
        tc qdisc replace dev "$dev" root netem \
            delay "${HALF_RTT_MS}ms" rate "${RATE_MBIT}mbit" limit 10000
    else
        tc qdisc replace dev "$dev" root netem \
            delay "${HALF_RTT_MS}ms" rate "${RATE_MBIT}mbit" \
            loss random "${LOSS_PCT}%" seed "$SEED" limit 10000
    fi
}
...
while [ "$i" -lt 100 ] && ! grep -F 'backend-ready ' "$OUT/tcp-shift/backend.stdout" >/dev/null 2>&1; do
    kill -0 "$BACKEND_PID" 2>/dev/null || {
        cat "$OUT/tcp-shift/backend.stderr" >&2 || true
        echo "benchmark backend exited before ready" >&2
        exit 1
    }
    i=$((i + 1))
    sleep 0.05
done
grep -F 'backend-ready ' "$OUT/tcp-shift/backend.stdout" >/dev/null || {
    echo "timed out waiting for benchmark backend" >&2
    exit 1
}
...
while [ "$i" -lt 100 ]; do
    if ip link show "$TUN_NAME" >/dev/null 2>&1 &&
       grep -F "tcp-shift-p2: ready tun=$TUN_NAME" \
           "$OUT/tcp-shift/runtime.stdout" >/dev/null 2>&1; then
        break
    fi
    kill -0 "$RUNTIME_PID" 2>/dev/null || {
        cat "$OUT/tcp-shift/runtime.stdout" >&2 || true
        cat "$OUT/tcp-shift/runtime.stderr" >&2 || true
        echo "tcp-shift benchmark runtime exited before ready" >&2
        exit 1
    }
    i=$((i + 1))
    sleep 0.05
done
[ "$i" -lt 100 ] || { echo "timed out waiting for tcp-shift benchmark runtime" >&2; exit 1; }
...
tc qdisc replace dev "$TUN_NAME" root netem delay "${HALF_RTT_MS}ms" limit 10000
...
if ! wait "$BACKEND_PID"; then
    BACKEND_PID=
    cat "$OUT/tcp-shift/backend.stderr" >&2 || true
    echo "benchmark backend failed" >&2
    exit 1
fi
BACKEND_PID=
...
if ! wait "$RUNTIME_PID"; then
    RUNTIME_PID=
    cat "$OUT/tcp-shift/runtime.stderr" >&2 || true
    echo "tcp-shift benchmark runtime failed" >&2
    exit 1
fi
RUNTIME_PID=
...
[ -n "$TS_CWND" ] && [ -n "$TS_LOSS" ] && [ -n "$TS_TIMEOUT" ] || {
    echo "missing tcp-shift CUBIC metrics" >&2
    exit 1
}

# Remove tcp-shift shaping objects before the Linux reference phase.
tc qdisc del dev "$TUN_NAME" root >/dev/null 2>&1 || true
tc qdisc del dev "$TUN_NAME" ingress >/dev/null 2>&1 || true
tc qdisc del dev "$IFB_NAME" root >/dev/null 2>&1 || true
ip link del "$IFB_NAME" >/dev/null 2>&1 || true
...
# server->client is the measured data path; client->server carries ACKs.
if [ "$LOSS_PCT" = 0 ] || [ "$LOSS_PCT" = 0.0 ]; then
...
ip netns exec "$NS_CLIENT" tc qdisc replace dev "$VETH_CLIENT" root netem \
    delay "${HALF_RTT_MS}ms" limit 10000
...
server.close()
print(
    f"linux-server cc={cc} bytes={length} sha256={digest} "
    f"snd_cwnd_packets={snd_cwnd} rtt_us={rtt_us} total_retrans={total_retrans}",
    flush=True,
)
PY
LINUX_SERVER_PID=$!
...
while [ "$i" -lt 100 ] && ! grep -F 'linux-ready cc=cubic' "$OUT/linux/server.txt" >/dev/null 2>&1; do
    kill -0 "$LINUX_SERVER_PID" 2>/dev/null || {
        cat "$OUT/linux/server.stderr" >&2 || true
        echo "Linux CUBIC server exited before ready" >&2
        exit 1
    }
    i=$((i + 1))
    sleep 0.05
done
grep -F 'linux-ready cc=cubic' "$OUT/linux/server.txt" >/dev/null || {
    echo "timed out waiting for Linux CUBIC server" >&2
    exit 1
}

run_receiver "$LINUX_SERVER_IP" "$LINUX_PORT" "$OUT/linux/client.txt" "$NS_CLIENT"

if ! wait "$LINUX_SERVER_PID"; then
    LINUX_SERVER_PID=
    cat "$OUT/linux/server.stderr" >&2 || true
    echo "Linux CUBIC server failed" >&2
    exit 1
fi
LINUX_SERVER_PID=
...
[ -n "$TS_GOODPUT" ] && [ -n "$LINUX_GOODPUT" ] && [ -n "$LINUX_CWND" ] || {
    echo "missing benchmark result metrics" >&2
    exit 1
}
...
print(
    f"cubic_linux_benchmark=ok case={case} rtt_ms={rtt_ms} "
    f"rate_mbit={rate_mbit} loss_pct={loss_pct} payload_bytes={payload} "
    f"bdp_bytes={bdp} tcp_shift_goodput_mbps={ts:.6f} "
    f"linux_goodput_mbps={linux:.6f} ratio={ratio:.6f} "
    f"tcp_shift_cwnd_bytes={ts_cwnd} tcp_shift_loss_events={ts_loss} "
    f"tcp_shift_timeout_events={ts_timeout} linux_cwnd_packets={linux_cwnd} "
    f"linux_rtt_us={linux_rtt_us} linux_total_retrans={linux_retrans}"
)
PY
